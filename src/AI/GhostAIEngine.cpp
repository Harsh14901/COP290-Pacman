...
void GhostAIEngine::update(int mPosX,int mPosY,Direction _direction,EnemyState enemyState){
    this->mPosX = mPosX;
    this->mPosY = mPosY;
...
Direction GhostAIEngine::updateDirection(){
  ...
  if (available_directions.size() != 0) {
    ...
    if(type==0){
        ...
        return reverseDirectionIfNecessary(findBestDirection(targetX,targetY,available_directions));
    }else if(type==1 && rand()%4!=0){
        // Pinky: Target Position: 4 Tiles Ahead of Pacman 25% times take random direction
        
        int offX = pacman._direction==Direction::LEFT?-4:pacman._direction==Direction::RIGHT?4:0;
        int offY = pacman._direction==Direction::UP?-4:pacman._direction==Direction::DOWN?4:0;
        ...
        // TODO: Fix this
        int offX = pacman._direction==Direction::LEFT?-2:pacman._direction==Direction::RIGHT?2:0;
        int offY = pacman._direction==Direction::UP?-2:pacman._direction==Direction::DOWN?2:0;
        int int_targetX = pacman.mPosX /32  + offX;
        int int_targetY = pacman.mPosY /32  + offY; 
        
    }else if(type==3){
        // Clyde: Target Position: If close, similar to Blinky, else in bottom left half
        int targetX = 2;
        int targetY = 32;
        if(abs(pacman.mPosX/32 - mPosX/32)+(abs(pacman.mPosY/32 - mPosY/32)) > 8){
...
Direction GhostAIEngine::findBestDirection(int targetX,int targetY,unordered_set<Direction> available_directions){
