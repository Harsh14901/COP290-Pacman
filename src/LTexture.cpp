...
void LTexture::setRenderer(SDL_Renderer* renderer) { _gRenderer = renderer; }
...
bool LTexture::loadFromRenderedText(std::string textureText,
  ...
  if (textSurface != NULL) {
    // Create texture from surface pixels
    mTexture = SDL_CreateTextureFromSurface(_gRenderer, textSurface);
    if (mTexture == NULL) {
...
void LTexture::render(int x, int y, SDL_Rect* clip, double angle,
  ...
  // Render to screen
  SDL_RenderCopyEx(_gRenderer, mTexture, clip, &renderQuad, angle, center,
                   flip);
}
