...
void VentGrid::teleportRandom() {
  ...
  while (true) {
    x = rand() % 4;
    if (vents[x].first == currentVent.first &&
        vents[x].second == currentVent.second)
      continue;
    break;
  }
  // isOpen = false;
...
pair<int, int> VentGrid::getTeleportLocation() {
  teleportRandom();
  isOpen = false;
  return pair<int, int>(teleportLocation.second * 32,
                        teleportLocation.first * 32);
}
...
SDL_Rect VentGrid::getRect(int i, int j) {
  ...
  if (isOpen) {  // && currentVent.first==i && currentVent.second==j){
    rect = {63 * (1 + openingAnimator.get_current_frame() < 3 ? 0 : 1), 0, 63,
            71};
  }
  return rect;
}
